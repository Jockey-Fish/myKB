...
router.get("/", authMiddleware, (req, res) => {
  try {
    ...
    const documents = result.documents.map((doc) => ({
      ...
      // 添加文件大小格式化
      filesize_formatted: formatFileSize(doc.filesize),
    }));
  ...
  try {
    ...
    success(
      res,
      {
        ...
        filesize_formatted: formatFileSize(document.filesize),
...
router.post("/batch-delete", authMiddleware, async (req, res) => {
  try {
    ...
    logger.info(
      `批量删除文档: 成功${results.success.length}个, 失败${results.failed.length}个`,
      { userId },
    );
